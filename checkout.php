...
// Include Xendit PHP SDK and Initialize
require 'vendor/autoload.php';
\Xendit\Xendit::setApiKey('xnd_public_development_zKKj4k6DOCRH1FtanFObxn8JvYlXYhm4qIMsuByG0DJiVXt2Q5y2agSVWVIn');

// Function to create a GCash payment
function create_gcash_payment($amount, $reference_id) {
    // Generate unique order ID and create GCash payment
    $unique_order_id = generate_unique_order_id($user_id);
    $amount = $total_amount;  // Replace with the actual amount

    $gcash_payment = create_gcash_payment($amount, $unique_order_id);

    // Redirect to GCash payment page if payment is pending
    if ($gcash_payment['status'] === 'PENDING') {
        header('Location: ' . $gcash_payment['actions']['mobile_web_checkout_url']);
        exit();
    } else {
        // Handle failure
    }
    ...
    $params = [
        ...
        'redirect_url' => 'https://redirect.me/goodstuff',
        'callback_url' => 'https://webhook.me/gethooked',
    ];

    $gcash_payment = \Xendit\EWallets::create($params);
...
// Fetch cart items for the user and calculate total
$total_amount = 0;
try {
    ...
    foreach ($cart_items as $item) {
        $total_amount += $item['price'];  // Assuming 'price' is a column in your `cart` table
    }
